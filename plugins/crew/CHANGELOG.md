...
- `bash-safety` no longer reads `rm -rf /any/absolute/path` as `rm -rf /` (#240).
- A Bash redirect to an unquoted absolute path outside the project, such as an out-of-tree build
  root, is no longer refused (#240).
