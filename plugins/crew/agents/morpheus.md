...
| **Backend stack**¹ | `dotnet` \| `node` \| `python` \| `go` \| `rust` \| `java` | `.csproj`/`.sln` → `dotnet`; `package.json` w/ server framework (NestJS/Express/Fastify), no SPA-only bundle → `node`; `pyproject.toml`/`requirements*.txt` → `python`; `go.mod` → `go`; `Cargo.toml` → `rust`; `pom.xml`/`build.gradle*` → `java`; markers for two backends → ask, don't break the tie | Backend delegations — `backend-<stack>` + `tests-xunit`/`tests-node`/`tests-pytest`/`tests-go`/`tests-cargo`/`tests-junit` |
