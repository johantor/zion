...
- **audit `<scope>`** — read-only. Enumerate within the scope, classify, rank, and return a
  capped report (~12 findings) where each finding is a ready-to-run `/crew:debt <pointer>`.
  Never edit in audit mode.
- **open `<pointer>`** — fix mode. Classify, enumerate, gate, delegate, verify, commit — or, for
  a tier-2 project the user asks to outline, write the outline and stop. Resume a matching
  batch ledger if one exists (*The batch ledger*, below).

If the mode is ambiguous, ask before doing anything.
...
1. Parse the scope; a bare invocation gets a usage hint. Valid scopes: a path, a lane
   ...
   or `diff` (files changed on the current branch vs base).
2. Enumerate with `grep`/`rg` — counts and file lists only (`context-discipline`). `diff`:
   `git diff --name-only <base>...HEAD`, filtered by lane. `stale`: the **Stale-suppression
   heuristic** — grep-only candidates, never compile. `outdated`: each stack's discover-outdated
   command, then triage each `current → target` delta by the **Upgrade workflow** risk levels.
   Metadata only — never install, restore or build.
3. Classify each finding by the rubric.
4. **Drop justified findings from the report, keep them in the totals** (*Justified
   suppressions*): a meaningfully justified suppression, or one a project policy section
   excludes, is not listed but is counted. `stale` is exempt, and skipped tests are never
   excluded. Unclear intent → surface the finding. Audit has no `--force`: excluded sites are
   listed under *Excluded from the ranking* with their rationale instead.
5. Rank trivially-fixable → needs-real-work → needs-investigation (`outdated`: SAFE → REVIEW →
   CAUTION); within a tier, smaller blast radius first.
6. Cap at ~12. 50+ for one rule is one entry: "50+ for rule X — run `/crew:debt X`". For
   `outdated`, keep the cap by risk rank and fold the tail into "N more outdated".
7. One line per finding: classification, count, an evidence pointer (`file:line`, a grep
   command, or the `current → target` delta), and the `/crew:debt` invocation.
8. **Lead with the totals line**: `N findings (J justified, P excluded by policy) — M shown`,
   omitting a zero term. Never report a scope as clean when exclusion is the only reason nothing
   is listed.
...
5. **Gate** by the **Blast-radius gate**, report radius and classification, then route: within
   ...
   report and stop, never pin or force.
...
7. **Delegate** one worker per lane per batch, by lane owner: backend → `crew:tank`, frontend →
   `crew:trinity`, a skipped unit test → `crew:oracle`, a skipped e2e spec → `crew:dozer`.
   Independent batches go out in parallel. Flip each to `in-progress` as it launches. A
   mechanical, behavior-preserving batch runs with `model: sonnet`; a run-and-report re-check
   with `model: haiku`. First snapshot per-mechanism suppression counts, **and which sites carry
   a justification**, across the batch's exact file list — step 8 checks against this. Each
   handoff carries:
   - the exact file list, the stack, and "load `debt-taxonomy-<stack>`";
...
8. **Verify** each return against your own re-sweep, not the worker's counts: the targeted
   ...
   the snapshot; no surviving suppression **gained** a justification (one that left with its
   suppression is expected). Reject and re-delegate on a miss, recording each round in
...
- Each batch: `id:`, `status:` `pending`|`in-progress`|`done`|`blocked`, `lane:`,
  `acceptance:`, `attempts:`, and `evidence:` — the commit SHA first when `done`, the attempt
  history when `blocked`.
...
unconfirmed: re-verify it per step 8, and reset it to `pending` if unmet. Continue from the first
batch not `done`. A `blocked` batch stays blocked until the user decides. All `done` → one line
(`<pointer> already complete — N/N batches done.`) and stop. Ask only when git contradicts the
ledger.
