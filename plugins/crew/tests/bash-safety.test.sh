...
# The floor does not decide WHOSE rename it is: with another plugin installed, both
# guards fire on every Bash call, and this one must not refuse that plugin's owner.
assert_allow "another plugin's git owner may git mv" "$HOOK" "$(payload_bash 'git mv src/a.ts src/b.ts' other-owner)"
