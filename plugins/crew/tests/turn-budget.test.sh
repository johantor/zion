...
assert_allow "an agent not on crew's roster is never warned" turn-budget.sh "$(payload_post Explore /tmp/t/explore.jsonl)"
...
found=""
for f in "$CREW_TURN_BUDGET_DIR"/crew-turn-budget.*.Explore; do [ -e "$f" ] && found="$f"; done
