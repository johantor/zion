...
- The shared guard library's raw-read rule let `cat <file> 2>/dev/null` through: the stderr
  redirect ended the end-of-command match, so a file that dumps into the context exactly like a
  bare `cat` got the opposite verdict. A trailing `2>`/`2>>` is now consumed before that match,
  and both spellings block. A `>` redirect still falls through, since it moves stdout out of the
  context.
