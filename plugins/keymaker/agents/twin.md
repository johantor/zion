...
- The delegation names the stack (`.NET` or `TypeScript`). **Load that stack's skill via the Skill tool** — `debt-taxonomy-dotnet` or `debt-taxonomy-typescript` — for the safe-removal recipe of the suppression mechanism named in the delegation. If the delegation omits the stack, ask `keymaker:keymaker` rather than guessing.
